...
export const getTimberLogsForStack = async (puulaaniId: number) => {
    ...
    const query = `
        SELECT
            ...
            ptl.puulaani_id,
            ptl.puutavara_nro,
            pt.puutavara AS timber_type_name, -- Joined from puutavarat
            ptl.purkupaikka_id,
            pp.purkupaikka AS unloading_site_name, -- Joined from purkupaikka
        ...
        WHERE ptl.puulaani_id = $1 
          AND ptl.jaljella > 0 -- Only fetch logs that have volume left
